...
def log_event(cur, status, content_hash, entries_updated, message):
    cur.execute("""
        ...
        VALUES (gen_random_uuid(), 'ofac_consolidated_ingestion', 'system', 'ofac_consolidated', 'scheduler', 'ofac_consolidated_scheduler', %s)
    """, (
        json.dumps({
            "status": status,
...
def run_once():
    ...
    try:
        ...
        log_event(cur, "updated", content_hash, inserted, f"Ingested {inserted} entries from updated Consolidated list.")
        conn.commit()
        cur.close()
        conn.close()
